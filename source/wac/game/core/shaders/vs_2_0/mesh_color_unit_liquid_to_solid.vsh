// (C)2013 S2 Games
// mesh_color_unit_liquid_to_solid.vsh
// 
// Unit vertex shader that shifts between a liquified morphing effect with scrolling normals
// and a standard rigid diffuse mesh
//=============================================================================
...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	// Shift function
	float fShiftTimeDivision = 1.0f / fShiftTime;
	float fShiftStayMul      = fStayTime * fShiftTimeDivision;
	float fLerp              = saturate(abs(fmod(fTime * fShiftTimeDivision, 2.0f * fShiftStayMul + 2.0f) - (fShiftStayMul + 1.0f)) - ((fShiftStayMul + 1.0f) * 0.5f - 0.5f));
	Out.Texcoord1.z          = fLerp;

	vPosition.xyz += (1.0f - fLerp) * vNormal * abs(fmod((fTime - In.Texcoord0.y) * fMorphSpeed, 2.0f) - 1.0f) * fMorphScale;
