...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	vPosition.rgb += vNormal * fFatScale;

	float3 vPositionOffset = mul(vPosition, mWorldViewOffset).xyz;
	
	Out.Position       = mul(vPosition, mWorldViewProj);
	Out.Color0         = vColor;
	Out.Texcoord0.xy      = (In.Texcoord0 + vUVScroll1 * fTime + vUVScroll2 * fTime) * vUVScale;
	Out.Texcoord0.wz      = ((1.0f - In.Texcoord0) + vUVScroll1 * fTime - vUVScroll2 * fTime) * vUVScale;
