...
VS_OUTPUT VS( VS_INPUT In )
{
	...
	float3 vWVTangentTer = float3(1.0f, 0.0f, 0.0f);
	vWVTangentTer -= vWVNormal * dot(vWVNormal, vWVTangentTer);
	vWVTangentTer = normalize(vWVTangentTer);
	float3 vWVBinormal = cross(vWVTangent, vWVNormal) * vInTangent.w;
	float3 vWVBinormalTerrain = cross(vWVTangentTer, vWVNormal);

	float3x3 mRotate = transpose(float3x3(vWVTangent, vWVBinormal, vWVNormal));
	float3x3 mRotateTerrain = transpose(float3x3(vWVTangentTer, vWVBinormalTerrain, vWVNormal));
	
	Out.SunLight.xyz  	 = mul(vLight, mRotate).xyz;
	Out.TerrainSunLight.xyz  = mul(vLight, mRotateTerrain).xyz;
	Out.HalfAngle    	 = mul(normalize(vLight + vCamDirection), mRotate);
	Out.TerrainHalfAngle     = mul(normalize(vLight + vCamDirection), mRotateTerrain);
#elif (LIGHTING_QUALITY == 2)
	float3 vLight = vSunPositionWorld.xyz;		
	float3 vWVNormal = mul(vNormal, mWorldRotate);
